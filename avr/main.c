...
int main(void)
{
	...
	for(;;)
		...
		{   
			...
			//PORTD |= (1 << PD5);   // switch is closed, switch BLUE LED on (Remove for Magenta)
        	}
        	else 
		{
		...
		{   	
			...
			//PORTD |= (1 << PD3);  // switch is closed, switch RED LED on (Remove comment for Yellow)
        	}
        	else 
		{
		...
		{   // is SWITCH_2 open?
			...
			//PORTD |= (1 << PD4);   // switch is closed, switch GREEN LED on (Remove comment for Cyan)
        	}
        	else 
		{
			PORTD &= ~(1 << PD5);  // switch is open, switch BLUE LED off
